...
import { AttachmentsSortTypes } from '../model/effects';
...
export const getRowsTemplate = (item) => ({
  ...
  values: [
    ...
    {
      value: item.vehicle ? Formatters.carNameFormatter(item.vehicle) : '-',
    },
    {
      value: item.driver?.userAccount ? Formatters.nameFormatter(item.driver.userAccount) : '-',
    },
    {
      value: item.createdBy ? Formatters.nameFormatter(item.createdBy) : '-',
    },
    {
...
export const getDeletePopupBody = (selectedItem) => {
  return (
    <p>
      ...
      <b>{selectedItem?.vehicle?.monitoringDevice?.name ?? ''}</b>?
    </p>
  );
};
