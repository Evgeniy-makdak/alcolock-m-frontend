...
export const getDeletePopupBody = (selectedItem) => {
  if (selectedItem?.vehicle) {
    return <p>
      ...
      Алкозамок <b>{selectedItem?.name}, {selectedItem?.serialNumber}</b> с
      привязанным к нему ТС <b>{selectedItem?.vehicle.manufacturer} {selectedItem?.vehicle.model}, {selectedItem?.vehicle.registrationNumber}, {selectedItem?.vehicle.year}</b>?
    </p>
  } else {
    return <p>
      Вы действительно хотите удалить
      Алкозамок <b>{selectedItem?.name}, {selectedItem?.serialNumber}</b>?
    </p>
  }
}
