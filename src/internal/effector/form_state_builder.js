...
export default class FormStateBuilder {
  constructor({ initialData, initialValidations = {}, getValidators = null }) {
    ...
    this.$currentAndPrevData = combine(this.$formData, this.$previousFormData, (formData, previousFormData) => {
      return {
    ...
    this.$isValidForm.on(this.setIsValidForm, (state, payload) => payload).reset(this.resetFormData);

    this.$formData
      .on(this.setFormDataField, (state, payload) => ({ ...state, [payload.key]: payload.value }))
      .reset(this.resetFormData);

    this.$previousFormData.on(this.setPreviousFormData, (state, payload) => payload).reset(this.resetFormData);
    ...
    for (const key in initialData) {
      ...
      if (initialValidations && initialValidations[key]) {
        ...
        this.valueStateManager[key].$validationStore.watch((value) => this.setFormValidations({ value, key }));
      }
