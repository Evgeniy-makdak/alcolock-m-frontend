...
import {
  changeUserLoadingState, createUserLoadingState, lastGetUserDataRequest,
  lastSearchDriversRequest, lastSearchUsersRequest,
  lastUsersListRequest, userBranchSwitchLoadingState,
  userDataLoadingState, userErrorState,
  usersLoadingState
} from "./store";
import UsersApi from "../../../data/api/users/users_api";
...
export const addUser = createEffect((data) => {
  ...
  const {promise} = UsersApi.createItem(UsersMapper.getUserToApi({...data, branchId: selectedBranch?.id ?? 10}))
